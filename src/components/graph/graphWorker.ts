import {type Simulation, type SimulationNodeDatum, forceSimulation, forceLink, forceManyBody, forceCenter} from "d3-force";
...
self.onmessage = (e: MessageEvent) => {
	...
	if (type === "init") {
		...
		const linkData: { source: string | Node; target: string | Node }[] =
			links.map((l: Link) => ({ ...l }));
