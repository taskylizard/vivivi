  ...
  useEffect(() => {
    ...
    const renderGraph = async () => {
      ...
      try {
        ...
        function renderLinksStatic() {
          linkRenderData.forEach((l) => {
            const linkData = l.simulationData
            l.gfx.clear()
            l.gfx.moveTo(
        ...
        function renderLabelsStatic() {
          nodeRenderData.forEach((n) => {
            const { x, y } = n.simulationData
            if (!x || !y) return
            n.label.position.set(x + width / 2, y + height / 2)
            n.label.alpha = 1 // Ensure labels are fully visible
          })
        }
